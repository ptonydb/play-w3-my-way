...
class CR4CharacterMenu extends CR4MenuBase
{
	...
	private var m_fxSetTabsLabelText	 : CScriptedFlashFunction;//zur13 modSSS
	public  var currTab  				 : int; //zur13 modSSS
	// sortX - begin
	...
	public  var tabChanging 			 : bool; //zur13 modSSS
	...
	{	
		...
		m_fxSetTabsLabelText = m_flashModule.GetMemberFlashFunction( "sssSetTabsLblText" );//zur13 modSSS
		...
		GetWitcherPlayer().UpdateMutationSkillSlots();
	...
	event  OnCloseMutationPanel():void
	{
		...
		GetWitcherPlayer().UpdateMutationSkillSlots();
		UpdateData( true );
		UpdateMasterMutation(true); // sortX
		UpdateAllMutationsDataForMaster();
		BlockClosing( false );
	}
	...
	private function UpdateMasterMutation(optional updateMasterMutationDesc : bool):void
	{
		var currentlyEquipped : array<EPlayerMutationType>; // sortX
		...
		if( GetWitcherPlayer().IsMutationSystemEnabled() )
		{
			...
			SSS_UpdateMasterMutation( this ); //zur13 modSSS
			// sortX - begin
	...
	// sortX - begin
	private function SetMasterMutationColors()
	{
		var currentlyEquipped : array<EPlayerMutationType>; // sortX
	...
	event  OnEquipMutation( mutationId : int )
	{
		...
		if( !SSS_IsMutationEquipped(mutationId, GetWitcherPlayer().SSS_GetEquippedMutationType()) && SSS_CanEquipMutation(this, mutationId, GetWitcherPlayer().SSS_GetEquippedMutationType()) ) //zur13 modSSS
		{
			if( thePlayer.IsInCombat() )
			{
				showNotification( GetLocStringByKeyExt( "menu_cannot_perform_action_combat" ) );
	...
	event  OnUnequipMutation( mutationId : int ) // sortX
	{
		// sortX - begin
		...
		if( currentlyEquipped.Size() > 0 ) // sortX
		{
			if( thePlayer.IsInCombat() )
			{
				showNotification( GetLocStringByKeyExt( "menu_cannot_perform_action_combat" ) );
	...
	private function UpdateTargetMutationData( mutationId : EPlayerMutationType, optional updateMasterMutationDesc : bool ):void
	{
		var mutationData      : CScriptedFlashObject;
	...
	private function UpdateAllMutationsData():void
	{
		...
		m_flashValueStorage.SetFlashArray( "character.mutations.list", mutationsList ); UpdateMasterMutation(); //zur13 modSSS
	}
	// sortX
	private function UpdateAllMutationsDataForMaster():void
	{
		var mutationsList      : CScriptedFlashArray;
		var mutationData       : CScriptedFlashObject;
		var count              : int;
	...
	private function CreateMutationFlashDataObj( curMutationId : EPlayerMutationType ) : CScriptedFlashObject
	{
		...
		var equippedMutationId : array<EPlayerMutationType>; // sortX
	...
	event  OnBuySkill(skill : ESkill, slotID : int)
	{
		...
		{   slotID = SSS_ConvertTabSlotIDToSlotID(slotID, SSS_GetCurrentTab()); //zur13 modSSS XXX skill can be on other tab?
			tryUnequipSkill(skill);
			thePlayer.AddSkill(skill);
			thePlayer.EquipSkill(skill, SSS_ConvertSlotIDToTabSlotID(slotID, SSS_GetCurrentTab())); //zur13 modSSS XXX skill can be on other tab?
	...
	event  OnSwapSkill(skill1 : ESkill, slotID1 : int, skill2 : ESkill, slotID2 : int)
	{
		...
		{   slotID1 = SSS_ConvertTabSlotIDToSlotID(slotID1, SSS_GetCurrentTab()); slotID2 = SSS_ConvertTabSlotIDToSlotID(slotID2, SSS_GetCurrentTab()); //zur13 modSSS
	...
	event  OnEquipSkill(skill : ESkill, slotID : int)
	{
		...
		{   slotID = SSS_ConvertTabSlotIDToSlotID(slotID, SSS_GetCurrentTab()); //zur13 modSSS
	...
	{	
		var skill : ESkill;
		if(SSS_CheckTabChange(slotID)) return true; //zur13 modSSS
		if (thePlayer.IsInCombat())
		{
			showNotification(GetLocStringByKeyExt("menu_cannot_perform_action_combat"));
			OnPlaySoundEvent("gui_global_denied");
	...
	event  OnUpgradeSkill(skillID : ESkill)
	{
		...
		{	slotFrom = SSS_ConvertMutSlotIDFromFlash(slotFrom, SSS_GetCurrentTab()); slotTo = SSS_ConvertMutSlotIDFromFlash(slotTo, SSS_GetCurrentTab()); //zur13 modSSS mutTabs
	event  OnEquipMutagen(itemID:SItemUniqueId, slotId:EEquipmentSlots)
	{
		...
		{	slotId = SSS_ConvertMutSlotIDFromFlash(slotId, SSS_GetCurrentTab()); //zur13 modSSS mutTabs
	...
	protected function tryUnequipSkill(skill : ESkill):void
	{
		...
		else
		{
			...
			if( SSS_IsSlotOnCurrentTab( currentSkillSlotIdx + 1, this ) ) //zur13 modSSS
			m_fxClearSkillSlot.InvokeSelfOneArg(FlashArgInt(SSS_ConvertSlotIDToTabSlotID(currentSkillSlotIdx + 1, SSS_GetCurrentTab()))); //zur13 modSSS
	...
	protected function CreateBonusGFxData(index:int):CScriptedFlashObject
	{
		...
		return SSS_CreateBonusGFxData(index);//zur13 modSSS
	...
	{		
		...
		SSS_UpdateAppliedSkills(); return;//zur13 modSSS
	...
	protected function UpdateAppliedSkillIfEquipped(skill : ESkill):void
	{
		...
		if( !SSS_IsSlotOnCurrentTab(slotId, this) ) return;//zur13 modSSS
		if (slotId != -1)
		{
			UpdateAppliedSkill(slotId);
		}
	...
	protected function SendEquippedSkillInfo(curSlot : SSkillSlot):void
	{
		...
		var equippedMutationId : array<EPlayerMutationType>; // sortX
		var equippedMutation   : SMutation;
		if( !SSS_IsSlotOnCurrentTab(curSlot.id, this) ) return;//zur13 modSSS
		equippedMutationId = GetWitcherPlayer().GetEquippedMutationType();
		// sortX - begin
		if( equippedMutationId.Size() > 0 )
		{
		...
		gfxSlot.SetMemberFlashInt('slotId', SSS_ConvertSlotIDToTabSlotID(curSlot.id, SSS_GetCurrentTab())); //zur13 modSSS
		...
		colorBorderId = "";
		if( equippedMutationId != EPMT_None && curSlot.id >= BSS_SkillSlot1 && curSlot.id <= BSS_SkillSlot4 ) //zur13 modSSS
		{
			colorsList = equippedMutation.colors;
			...
			if( !SSS_TargetDisableMutationSkillSlotsColorRestrictions() )//zur13 modSSS
			// sortX - begin
			if( colorsList.Contains(SC_Yellow) )
			{
				colorBorderId += "Yellow";
	...
	protected function UpdateMutagens():void
	{
		...
		if( SSS_UpdateMutagens2() ) return; //zur13 modSSS mutTabs
		...
		for (idx = 0; idx < mutCount; idx+=1)
		{
			...
			{
				playerInv.SetInventoryFlashObjectForItem(currentMutSlot.item, gfxMutSlot); gfxMutSlot.SetMemberFlashString('color', SSS_GetMutageSlotColor(currentMutSlot, this, invComponent)); //zur13 modSSS
				//gfxMutSlot.SetMemberFlashString('color', invComponent.GetSkillMutagenColor(currentMutSlot.item)); //zur13 modSSS
			}
			else
			{
	...
	private function GetSkillTooltipData(targetSkill : ESkill, compareItemType : int, isGridView : bool)
	{
		...
		targetSubPathLocName += SSS_GetSkillTooltipStringSuffix(targetSkill);//zur13 modSSS
		...
		skillLevelString = skillLevelString + SSS_GetSkillTooltipSynergySufix(targetSkill);//zur13 modSSS
	...
	event  OnGetItemData(item : SItemUniqueId, compareItemType : int)
	{
		...
		if( tooltipInv.IsIdValid(item) )
		{
			...
			resultData.SetMemberFlashString("ItemName", itemName + SSS_GetItemTooltipSynergySufix(item, this)); //zur13 modSSS
	...
	//zur13 modSSS public morozov antipattern ;)
	public function SSS_tryUnequipSkill(skill : ESkill):void
	{
		tryUnequipSkill(skill);
	}
	
	public function SSS_PopulateTabData(tabIndex:int) : void
	{
		PopulateTabData(tabIndex);
	}
	
	public function SSS_GetTabForSkill(skillID : ESkill) : CharacterMenuTabIndexes
	{
		return GetTabForSkill(skillID);
	}
	
	public function SSS_UpdateAppliedSkill(slotID:int):void
	{
		UpdateAppliedSkill(slotID);
	}
	
	public function SSS_UpdateGroupsData():void
	{
		UpdateGroupsData();
	}
	
	public function SSS_UpdateMutagens():void
	{
		UpdateMutagens();
	}
	
	public function SSS_UpdatePlayerStatisticsData()
	{
		UpdatePlayerStatisticsData();
	}
	
	public function SSS_Get_m_fxPaperdollChanged():CScriptedFlashFunction
	{
		return m_fxPaperdollChanged;
	}
	
	public function SSS_Get_m_fxClearSkillSlot(): CScriptedFlashFunction
	{
		return m_fxClearSkillSlot;
	}
	...
	public function SSS_UpdateSkillPoints() : void
	{
		UpdateSkillPoints();
	}
	
	public function SSS_UpdateTargetMutationData( mutationId : EPlayerMutationType ):void
	{
		return UpdateTargetMutationData( mutationId );
	}
	
	public function SSS_CheckTabChange(slotID:int):bool
	{
		if(slotID==2004) 
		{
			//Prev tab
			...
			tabChanging = true;
			SSS_PrevTab(this);
			SSS_UpdateTabTextAndRefreshGUI();
			tabChanging = false;
			return true;
		}
		if(slotID==2005) 
	...
	public function SSS_UpdateTabTextAndRefreshGUI()
	{
		var tab: int;
		tab = SSS_GetCurrentTab();
		SSS_SetTabsText(tab+"/"+SSS_GetTabsCountFromSlotsCount());
	...
	public function SSS_SetTabsText(text:string)
	{
		m_fxSetTabsLabelText.InvokeSelfOneArg( FlashArgString( text ) );
	}
	
	public function SSS_InitCurrentTab()
	{
		// Init current
		currTab = ( ( W3PlayerAbilityManager ) GetWitcherPlayer().abilityManager ).SSS_GetCurrentTab();
		if(!currTab || currTab < 1 || currTab > SSS_GetTabsCountFromSlotsCount() || !SSS_TargetRememberLastOpenedTab() ){
			currTab = 1;
		}
	}
	
	public function SSS_UpdatePamCurrentTab()
	{
		// update current tab in player ability manager
		//if(!currTab)
		//{
		( ( W3PlayerAbilityManager ) GetWitcherPlayer().abilityManager ).SSS_UpdateCurrentTab(currTab);
		//}
	}
	
	public function SSS_GetCurrentTab():int
	{
		if(!currTab)
		{
			SSS_InitCurrentTab();
	...
	public function SSS_IncrementAndGetCurrrentTab():int
	{
		if(!currTab)
		{
			SSS_InitCurrentTab();
		...
		if( currTab > SSS_GetTabsCountFromSlotsCount() )
		{
			currTab = 1;
		}
		SSS_UpdatePamCurrentTab();
		return currTab;
	}
	
	public function SSS_DecrementAndGetCurrrentTab():int
	{
		if(!currTab)
		{
			SSS_InitCurrentTab();
		...
		if( currTab < 1 )
		{
			currTab =  SSS_GetTabsCountFromSlotsCount();
		}
		if(currTab < 1)
		{
			currTab = 1;
	...
	public function SSS_FixSynergyBonuses()
	{   
		var color         			  : ESkillColor;
		var i, g1, g2, g3, g4, ds, mt : int;
		
		if( SSS_TargetMutagenSlotsTabbed() )
		{
			ds = SSS_DeSynergyGroupNum();
			g1 = SSS_ConvertMutGroupIDFromFlash(1, SSS_GetCurrentTab());
			g2 = SSS_ConvertMutGroupIDFromFlash(2, SSS_GetCurrentTab());
			g3 = SSS_ConvertMutGroupIDFromFlash(3, SSS_GetCurrentTab());
			g4 = SSS_ConvertMutGroupIDFromFlash(4, SSS_GetCurrentTab());
			mt = ((W3PlayerAbilityManager)thePlayer.abilityManager).SSS_GetMutationSkillGroupID();
			
			for( i = 1; i <= SSS_TargetSlotsTabs() * 4 + 1; i += 1 )
			{
				if( i > 0 && i != ds && i != g1 && i != g2 && i != g3 && i != g4 && i != mt )
				{
					GetGroupBonusDescription(i, color); //zur13 modSSS mutTabs this method adds groups synergy bonus to PlayerAbilityManager
				}
			}
		}
	}
	
	public function SSS_CreateBonusGFxData(index:int):CScriptedFlashObject
	{
		var gfxGroupBonus : CScriptedFlashObject;
		var description   : string;
		var color         : ESkillColor;
		//groupId = SSS_ConvertMutGroupIDFromFlash(groupId, SSS_GetCurrentTab()); //zur13 modSSS mutTabs
		//description = GetGroupBonusDescription(index, color);
		description = GetGroupBonusDescription(SSS_ConvertMutGroupIDFromFlash(index, SSS_GetCurrentTab()), color);
		if (index > m_previousSkillBonuses.Size())
		{
			m_previousSkillBonuses.PushBack(description);
		}
	...
	public function SSS_UpdateAppliedSkills():void
	{		
		var gfxSlots           : CScriptedFlashObject;
		var gfxSlotsList       : CScriptedFlashArray;
		var curSlot            : SSkillSlot;
		...
		LogChannel( 'CHR', "UpdateAppliedSkills add " + slotsCount + " items" );
		for( i=tabFirstSlotId - 1; i < nextTabFirstSlotId - 1; i+=1 )
		{
			curSlot = skillSlots[i];
			equipedSkill = thePlayer.GetPlayerSkill( curSlot.socketedSkill );
			...
			gfxSlots.SetMemberFlashInt( 'tabId', GetTabForSkill( curSlot.socketedSkill ) );
			gfxSlots.SetMemberFlashInt( 'slotId', SSS_ConvertSlotIDToTabSlotID(curSlot.id, tab) );
			gfxSlots.SetMemberFlashInt( 'unlockedOnLevel', curSlot.unlockedOnLevel );
			gfxSlots.SetMemberFlashInt( 'groupID', curSlot.groupID );
			gfxSlots.SetMemberFlashBool( 'unlocked', curSlot.unlocked );
		...
		if( tab != 1 ) 
		{
			for( i = BSS_SkillSlot1 - 1; i < BSS_SkillSlot4; i+=1 )
			{
				curSlot = skillSlots[i];
				equipedSkill = thePlayer.GetPlayerSkill( curSlot.socketedSkill );
	...
	public function SSS_UpdateMutagens2():bool
	{
		var idx               : int;
		var mutCount          : int;
		var slotUnlocked      : bool;
		...
		for (idx = SSS_GetTabFirstMutagenIdx(SSS_GetCurrentTab()); idx < SSS_GetTabFirstMutagenIdx(SSS_GetCurrentTab())+4; idx+=1) //zur13 modSSS mutTabs for (idx = 0; idx < mutCount; idx+=1)
		{
			currentMutSlot = skillMutagenSlots[idx];
			gfxMutSlot = m_flashValueStorage.CreateTempFlashObject();
			slotUnlocked = GetWitcherPlayer().GetLevel() >= currentMutSlot.unlockedAtLevel;
			
			gfxMutSlot.SetMemberFlashInt('slotId', SSS_ConvertMutSlotIDToFlash(currentMutSlot.equipmentSlot) );
			gfxMutSlot.SetMemberFlashInt('groupId', currentMutSlot.skillGroupID);
			gfxMutSlot.SetMemberFlashString('slotType', SSS_ConvertMutSlotIDToFlash(currentMutSlot.equipmentSlot) );
			gfxMutSlot.SetMemberFlashBool('unlocked', slotUnlocked);
			gfxMutSlot.SetMemberFlashInt('unlockedAtLevel', currentMutSlot.unlockedAtLevel);
